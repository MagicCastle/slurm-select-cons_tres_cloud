			  ...
			  will_run_data_t *will_run_ptr)
{
	...
	if (filtered && bit_set_count(filtered) > 0) {
		rc = _will_run_test_with_bitmap(job_ptr, filtered, min_nodes,
						max_nodes, req_nodes,
						job_node_req,
						preemptee_candidates,
						preemptee_job_list,
				   ...
				   will_run_data_t *will_run_ptr)
{
	...
	if ((mode != SELECT_MODE_TEST_ONLY) && (mode != SELECT_MODE_RUN_NOW))
		return EINVAL;

	if ((mode == SELECT_MODE_RUN_NOW) && preemptee_candidates &&
	    (list_count(preemptee_candidates) > 0)) {
				 ...
				 will_run_data_t *will_run_ptr)
{
	if (mode == SELECT_MODE_WILL_RUN) {
		...
		return _test_only(job_ptr, node_bitmap, min_nodes, max_nodes,
				  req_nodes, job_node_req);
	} else if (mode == SELECT_MODE_RUN_NOW) {
